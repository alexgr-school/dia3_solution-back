...
def prepare_data(network_df):
    ...
    for building_id, building_subdf in building_subdfs:
        list_infras = [
            dict_infras[infra_id] for infra_id in building_subdf["infra_id"].values
        ]
...
if __name__ == "__main__":
    ...
    pandas.DataFrame({"priority": priority_list, "id_bat": building_ids}).to_excel(
        "./data/priorities.xlsx", index=False
    )
